...
const lobbyPhotoUrls = Object.values(
	import.meta.glob('../../assets/lobbyp/*.webp', {
		eager: true,
		query: '?url',
		import: 'default',
	})
...
// The lobby photos are 4096 x 3072, so keep their frame at the same 4:3 ratio.
const LOBBY_PHOTO_ASPECT_RATIO = 4096 / 3072

function pickRandomPhotoIndex(currentIndex = -1) {
	if (lobbyPhotoUrls.length === 0) return -1
	if (currentIndex < 0 || lobbyPhotoUrls.length === 1) {
		return Math.floor(Math.random() * lobbyPhotoUrls.length)
	}

	let nextIndex = Math.floor(Math.random() * (lobbyPhotoUrls.length - 1))
...
export function buildLobbyRoom(ctx, lobby) {
	...
	function addLobbyWhiteboard({ leftItems, rightItems }) {
		...
		const logicalCanvasHeight = 1024
		// Keep the lobby emblem sharp while preserving the board's existing layout.
		const canvasScale = 1
		...
		if (ctx2) {
			const padX = 90
			const padY = 86
			const midW = Math.floor(logicalCanvasWidth * 0.34)
			const colW = (logicalCanvasWidth - padX * 2 - midW) / 2
			...
			function drawWelcome() {
				...
				const startY = logicalCanvasHeight / 2 - totalH / 2 + lineH * 0.85
			...
			function drawIconWatermark(img) {
				...
				const maxSize = Math.min(midW, logicalCanvasHeight) * 0.96
			...
			function drawList(x0, title, items, { align = 'left' } = {}) {
				...
				const safeItems = Array.isArray(items)
					? items
						.map((s) => String(s || '').trim().replace(/:\s*(?:Living Gallery|Live Meeting)\s*$/i, ''))
						.filter(Boolean)
					: []
				...
				const contentMaxH = Math.max(240, logicalCanvasHeight - yMargin * 2)
				...
				const yTop = (logicalCanvasHeight - totalH) / 2
				...
				for (let i = 0; i < shown.length; i += 1) {
					ctx2.fillText(`• ${shown[i]}`, x0, y + i * lineH)
				}
			...
			function renderBoard({ watermarkImg } = {}) {
				...
				drawList(logicalCanvasWidth - padX, '→→→→', rightItems, { align: 'right' })
...

	{
		...
		const frameH = boardH * 1.15
		const frameW = frameH * LOBBY_PHOTO_ASPECT_RATIO
		...
		let currentPhotoIndex = pickRandomPhotoIndex()
		let displayedPhotoTexture = currentPhotoIndex >= 0 ? textureLoader.load(lobbyPhotoUrls[currentPhotoIndex]) : null
		if (displayedPhotoTexture) displayedPhotoTexture.colorSpace = THREE.SRGBColorSpace

		const photoGeo = new THREE.PlaneGeometry(frameW + frameThickness * 2, frameH + frameThickness * 2)
		const photoMat = new THREE.MeshBasicMaterial({
			map: displayedPhotoTexture,
		...
		const pendingPhotoTextures = new Set()

		photo.userData.onClick = () => {
			const nextPhotoIndex = pickRandomPhotoIndex(currentPhotoIndex)
			if (nextPhotoIndex < 0 || nextPhotoIndex === currentPhotoIndex) return

			const loadVersion = ++photoLoadVersion
			let nextTexture
			nextTexture = textureLoader.load(
				lobbyPhotoUrls[nextPhotoIndex],
				() => {
					pendingPhotoTextures.delete(nextTexture)
					if (photoDisposed || loadVersion !== photoLoadVersion) {
						nextTexture.dispose()
						return
					}

					const previousTexture = displayedPhotoTexture
					displayedPhotoTexture = nextTexture
					currentPhotoIndex = nextPhotoIndex
					photoMat.map = nextTexture
			...
			pendingPhotoTextures.add(nextTexture)
		}
		...
		disposables.push(photoGeo, photoMat, {
			dispose() {
				photoDisposed = true
				photoLoadVersion += 1
				displayedPhotoTexture?.dispose()
				for (const texture of pendingPhotoTextures) texture.dispose()
				pendingPhotoTextures.clear()
			}
		})
	}
