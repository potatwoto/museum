...
export function getSharedRoomMaterialTextures({ width = 14, length = 18 } = {}) {
	if (!sharedRoomMaterialTextures) {
		const { mapTex: floorWoodMap, bumpTex: floorWoodBump } = makeWoodFloorTextures({ size: 512 })
		...
		const { mapTex: wallStuccoMap, bumpTex: wallStuccoBump } = makeStuccoWallTextures({ size: 512 })
		const { mapTex: ceilingStuccoMap, bumpTex: ceilingStuccoBump } = makeCeilingStuccoTextures({ size: 512 })

		const { mapTex: pillarMarbleMap, bumpTex: pillarMarbleBump } = makeMarbleTextures({ size: 512, seed: 0x6d617262 })
		const { mapTex: panelMarbleMap, bumpTex: panelMarbleBump } = makeMarbleTextures({ size: 512, seed: 0x70616e6c })
...
export function labelFromImageUrl(url) {
	...
	try {
		...
		const withoutBuildHash = withoutExt.replace(/-[A-Za-z0-9_-]{8}$/, '')
		const cleaned = withoutBuildHash.replace(/^File:/i, '').replace(/_/g, ' ').trim()
		return cleaned
	} catch {
		return ''
	}
...
export function makePlaqueTexture({ size = 512, text, aspect } = {}) {
	const maxW = typeof size === 'number' && Number.isFinite(size) ? Math.max(256, Math.floor(size)) : 512
